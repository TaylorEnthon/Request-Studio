...
Milestone 6 keeps asset workflows on the same boundary. Renderer calls named cURL import, Request export, code-generation, and Workspace export methods from the Preload whitelist; it never imports parsers, repositories, generators, serializers, SQL, or filesystem APIs. Import replaces credentials with deterministic placeholders and persists only empty secret-variable slots. Export and code generation share the Request Asset output sanitizer, which preserves placeholders while redacting credentials, credential-bearing JSON leaves, local paths, and file references.

Import and export previews are sender-scoped UUID capabilities. Successful import/save consumes the capability, and cross-sender IDs are rejected. UI request/workspace/language changes invalidate stale asynchronous results. Main owns save dialogs, filename sanitization, destination validation, temporary-file cleanup, and atomic replacement.

Workspace export contains only Workspace name, stable Collection references, sanitized Request assets, Environments, and variable slots. It excludes database IDs, timestamps, History, Experiments, Compare results, and Resources. Secret variables export with an empty value. Workspace export is currently one-way and its Repository snapshot remains in Main memory.
