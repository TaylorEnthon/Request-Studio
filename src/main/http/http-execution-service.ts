...
import { suggestFilename } from '../response/suggested-filename'

type Active={controller:AbortController;manual:boolean;timer:ReturnType<typeof setTimeout>}
type FileData={bytes:Buffer;filename:string}
type ServiceOptions={maximumBytes?:number;memoryThreshold?:number;responseDir?:string;resolveFile?:(ref:string)=>Promise<FileData>;resources?:ResponseResourceRegistry}
...
export class HttpExecutionService{
 ...
 private resolveFile?:ServiceOptions['resolveFile'];private memoryThreshold:number;private responseDir?:string;private resources?:ResponseResourceRegistry
 constructor(private db:Database.Database,options:ServiceOptions={}){this.maximumBytes=options.maximumBytes??50*1024*1024;this.memoryThreshold=options.memoryThreshold??10*1024*1024;this.responseDir=options.responseDir;this.resolveFile=options.resolveFile;this.resources=options.resources}
 ...
 private async run(executionId:string,draft:HttpRequestDraft,variables:EnvironmentValue[]){
  const active=this.active.get(executionId)!,started=Date.now(),startedAt=new Date(started).toISOString(),historyId=randomUUID();let responseData:any,errorData:any
  try{
   ...
   if(response.body)for await(const chunk of response.body){const bytes=chunk as Uint8Array;size+=bytes.byteLength;if(size>this.maximumBytes){if(filePath)await unlink(filePath).catch(()=>undefined);throw error('response_too_large','Response exceeded the maximum size.','response')}if(!filePath&&size>this.memoryThreshold&&this.responseDir){filePath=join(this.responseDir,draft.workspaceId,historyId,'response.bin');await mkdir(dirname(filePath),{recursive:true});await writeFile(filePath,Buffer.concat([...chunks.map(v=>Buffer.from(v)),Buffer.from(bytes)]));chunks.length=0}else if(filePath)await appendFile(filePath,bytes);else chunks.push(bytes)}
   let bytes=filePath?Buffer.alloc(0):Buffer.concat(chunks.map(v=>Buffer.from(v))),prefix=bytes.subarray(0,4096);const contentType=response.headers.get('content-type')||'';if(filePath){const handle=await open(filePath,'r');try{prefix=Buffer.alloc(Math.min(4096,size));const {bytesRead}=await handle.read(prefix,0,prefix.length,0);prefix=prefix.subarray(0,bytesRead)}finally{await handle.close()}}const classification=classifyResponse(contentType||null,prefix),kind=classification.kind
   if(['image','audio','video','pdf','binary'].includes(kind)&&!filePath&&this.responseDir){filePath=join(this.responseDir,draft.workspaceId,historyId,'response.bin');await mkdir(dirname(filePath),{recursive:true});await writeFile(filePath,bytes);bytes=Buffer.alloc(0)}
   const textual=['json','text','html','xml'].includes(kind),text=filePath?(textual?prefix.toString('utf8'):null):textual?bytes.toString('utf8'):null;if(kind==='json'&&text&&!filePath)JSON.parse(text)
   responseData={status:response.status,statusText:response.statusText,headers:Object.fromEntries(response.headers),kind,text,rawBase64:kind==='binary'&&!filePath?bytes.toString('base64'):null,sizeBytes:size,contentType,durationMs:Date.now()-started,filePath,classification,historyId}
  }catch(caught){
   ...
   const completed=new Date().toISOString(),safe=JSON.stringify(safeDraft(draft)),e=errorData?JSON.stringify({code:errorData.code,category:errorData.category,message:errorData.message}):null
   ...
   if(stale.length){this.db.transaction(()=>{for(const row of stale)this.db.prepare('DELETE FROM request_history WHERE id=?').run(row.id)})();for(const row of stale)if(row.response_file_path)void rm(dirname(row.response_file_path),{recursive:true,force:true}).catch(()=>undefined)}
  }
  if(responseData?.filePath&&this.resources&&['image','audio','video','pdf','binary'].includes(responseData.kind)){responseData.resource=await this.resources.register({historyId,source:'managed-response-file',kind:responseData.kind,declaredMimeType:responseData.classification.declaredMimeType,detectedMimeType:responseData.classification.detectedMimeType,effectiveMimeType:responseData.classification.effectiveMimeType,byteLength:responseData.sizeBytes,suggestedFilename:suggestFilename(responseData.headers['content-disposition']||null,draft.url,responseData.classification.effectiveMimeType),warnings:responseData.classification.warnings,path:responseData.filePath},historyId)}
  const safeResponse={...responseData};delete safeResponse.filePath;return {executionId,response:{...safeResponse,storedToFile:Boolean(responseData?.filePath)}}
 }
}
