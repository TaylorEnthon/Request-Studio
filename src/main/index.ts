...
protocol.registerSchemesAsPrivileged([{scheme:'request-studio-resource',privileges:{standard:true,secure:true,supportFetchAPI:true,stream:true}}])
...
function registerIpc(repo: Repository) {
  ...
  for (const [domain, table] of Object.entries(tableByDomain)) {
    ...
    ipcMain.handle(`${domain}:delete`, (_e, input) => { const checked=parse(z.object({id:z.string().min(1)}),input) as any;if(!checked)return invalid;if(domain==='workspaces'){httpService?.cleanupWorkspace(checked.id);repo.deleteWorkspace(checked.id)}else repo.delete(table,checked.id); return { ok:true,data:null } })
  }
