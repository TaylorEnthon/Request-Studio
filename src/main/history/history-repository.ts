...
export class HistoryRepository{
 ...
 delete(id:string,workspaceId:string){const row=this.get(id,workspaceId) as any;const changed=this.db.prepare('DELETE FROM request_history WHERE id=? AND workspace_id=?').run(id,workspaceId).changes>0;if(changed){this.removeManaged(row?.response_file_path);if(this.responseDir)try{rmSync(join(this.responseDir,workspaceId,id),{recursive:true,force:true})}catch{/* retry on startup */}}return changed}
 clear(workspaceId:string){const rows=this.list(workspaceId) as any[],changes=this.db.prepare('DELETE FROM request_history WHERE workspace_id=?').run(workspaceId).changes;for(const row of rows)this.removeManaged(row.response_file_path);if(this.responseDir)try{rmSync(join(this.responseDir,workspaceId),{recursive:true,force:true})}catch{/* retry on startup */}return changes}
